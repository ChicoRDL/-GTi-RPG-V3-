...
local snow = {}
-- default settings
settings = {type = "real", density = 1000, wind_direction = {0.01,0.01}, wind_speed = 2, snowflake_min_size = 1, snowflake_max_size = 3, fall_speed_min = 1, fall_speed_max = 4, jitter = true}
...
addEventHandler("onClientResourceStart",resourceRoot,function()
	...
	snow.cursor_highlight_checkbox = guiCreateCheckBox(100,18,206,19,"Enable snow shader",true,false,snow.window)
	...
	addEventHandler("onClientGUIClick",snow.accept_button,function(button,state)
		if button == "left" and state == "up" then
			...
			updateSnowJitter(guiRadioButtonGetSelected(snow.jitter_yes))
				exports.GTIhud:dm("Settings successfully saved!",255,0,0)
...
function loadSnowSettings()
	local file = xmlLoadFile("snow.xml")
	if not file then
...
function saveSnowSettings()
	local file = xmlLoadFile("snow.xml")
	if not file then
		file = xmlCreateFile("snow.xml","settings")
		if file then
	...
	if cursorhighlight then
		...
		xmlNodeSetValue(cursorhighlight,false)
	end	
