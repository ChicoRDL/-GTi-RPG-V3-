...
function toggleSnow(msg)
	if snowing then
		stopSnow()
		if msg then exports.GTIhud:dm("Snow successfully disabled!",255,0,0) end
	else
		startSnow() 
		if msg then exports.GTIhud:dm("Snow successfully enabled! You can use /snowsettings to adjust settings",0,255,0) end
	end
end
...
addEvent("GTIsnow.addSnow",true)
addEventHandler("GTIsnow.addSnow",resourceRoot,create)
--------------------------------
-- Command handler
--		Toggle via command
--------------------------------
...
function enableGroundSnow()
	...
	snowShader = dxCreateShader ( "snow_ground.fx", 2, maxEffectDistance )
