...
function updateShineDirection ()
	...
	for idx,v in ipairs( shineDirectionList ) do
		...
		if fhoursNow <= fhoursTo then
			...
			local msharpness, mbrightness = applyWeatherInfluence ( sharpness, brightness, nightness )
			...
			if myWaterShader then 
				...
				if not msharpness or not mbrightness then return end
				dxSetShaderValue( myWaterShader, "sSpecularPower", msharpness )
				dxSetShaderValue( myWaterShader, "sSpecularBrightness", mbrightness )
