...
addEventHandler("onIRCResourceStart",root,
	function ()
		function say (server,channel,user,command,...)
			...
			outputIRC("07[IRC] "..ircGetUserNick(user)..": "..message)
		end
		...
		addIRCCommandHandler("!adminlogupdate",
			function (server,channel,user,command,name,...)
				local message = table.concat({...}," ")
				if not name then ircNotice(user,"syntax is !adminlogupdate <message>") return end
				if message == "" then ircNotice(user,"syntax is !adminlogupdate <message>") return end
				exports.GTIres:outputWebLog("[IRC] "..ircGetUserNick(user)..": "..message)
			end
		)
