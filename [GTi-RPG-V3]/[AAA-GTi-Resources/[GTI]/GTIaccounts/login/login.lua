...
local isLoggedIn = false-- Is Player Logged In?
...
function loginPlayer(button, state)
	...
	if (isTransferBoxActive()) then
		outputLoginMessage("Notice: Please wait while server files are being downloaded", 255, 125, 0)
		return
	end
	...
	guiSetEnabled(login.button[1], false)
...
function outputLoginMessage(message, r, g, b)
	guiSetEnabled(login.button[1], true)
	guiSetText(login.label[4], message)
	guiLabelSetColor(login.label[4], r, g, b)
	guiSetText(login.label[5], message)
end
...
addEventHandler("onClientGUIClick", login.button[2], showRegisterPanel, false)
