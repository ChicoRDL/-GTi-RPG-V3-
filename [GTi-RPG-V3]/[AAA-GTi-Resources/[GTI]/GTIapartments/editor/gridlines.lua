...
-- Date: 14 Dec 2015
-- Resource: GTIapartments/gridlines.lua
-- Type: Client Side
-- Author: Ares
----------------------------------------->>
...
local function renderGridlines()
	g_dragElement = getDragElement()
	if ( not g_dragElement ) then return end
	
	local x,y,z = getElementPosition(g_dragElement)

	local minX,minY,minZ,maxX,maxY,maxZ = getElementBoundingBox ( g_dragElement )
	if not minX then
		local radius = getElementRadius ( g_dragElement )
		if radius then
	...
	--
	local elementMatrix = ( getElementMatrix(g_dragElement) and matrix(getElementMatrix(g_dragElement)) )
...
function drawXYZLines()
	g_dragElement = getDragElement()
	if not isElement(g_dragElement) then return end
	local camX,camY,camZ = getCameraMatrix()
	if getElementDimension(g_dragElement) ~= getElementDimension(localPlayer) then return end
	local radius = (getElementRadius(g_dragElement) or .3)*1.2
	local x,y,z = getElementPosition(g_dragElement)
	local xx,xy,xz = getPositionFromElementAtOffset(g_dragElement,radius,0,0)
	local yx,yy,yz = getPositionFromElementAtOffset(g_dragElement,0,radius,0)
	local zx,zy,zz = getPositionFromElementAtOffset(g_dragElement,0,0,radius)
