...
colorPicker = {
	...
	constructor = function( info )
		...
		local screenW, screenH = (screenW/2)-(info.selectWindow.width/2),(screenH/2)-(info.selectWindow.height/2)
		colorPicker.selectWindow = info.selectWindow
		colorPicker.GUI.selectWindow = guiCreateWindow(screenW, screenH, info.selectWindow.width, info.selectWindow.height, "Pick a color (click outside window to close)", false)
		...
		colorPicker.GUI.alphaBar = guiCreateStaticImage(colorPicker.selectWindow.paletteX + 255 + colorPicker.selectWindow.alphaOffset, colorPicker.selectWindow.paletteY,
		                                         colorPicker.selectWindow.alphaWidth, 255, "files/alpha.png", false, colorPicker.GUI.selectWindow)
...
--This is where the color updates every time you click the palette or presets!
	...
	closeSelect = function()
		...
		colorPicker.addCurrentColorToHistory()
			-- Change Group Color
		changeGroupColor(colorPicker.value[1], colorPicker.value[2], colorPicker.value[3])
...
--Wont work until I get the actual mouse working
		...
		local bx, by = wx + 18, wy + 394
		local color = tocolor(colorPicker.value[1], colorPicker.value[2], colorPicker.value[3], 255)
		dxDrawRectangle(bx, by, 312, 20, color, true)
		local tx1, ty1, tx2, ty2 = bx, by, bx + 312, by + 20
		dxDrawText("Color Preview", tx1-1, ty1-1, tx2-1, ty2-1, tocolor(0, 0, 0, 255), 1, "default-bold", "center", "center", false, false, true)
		dxDrawText("Color Preview", tx1-1, ty1+1, tx2-1, ty2+1, tocolor(0, 0, 0, 255), 1, "default-bold", "center", "center", false, false, true)
		dxDrawText("Color Preview", tx1+1, ty1-1, tx2+1, ty2-1, tocolor(0, 0, 0, 255), 1, "default-bold", "center", "center", false, false, true)
		dxDrawText("Color Preview", tx1+1, ty1+1, tx2+1, ty2+1, tocolor(0, 0, 0, 255), 1, "default-bold", "center", "center", false, false, true)
		dxDrawText("Color Preview", tx1, ty1, tx2, ty2, tocolor(255, 255, 255, 255), 1, "default-bold", "center", "center", false, false, true)
