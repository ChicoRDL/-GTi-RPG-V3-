...
function suicide(player,override,reason)
	...
	local cops = {}	
	if not exports.GTIemployment:getPlayerJob(player, true) == "Police Officer" and exports.GTIpoliceWanted:isPlayerWanted(player) then
		for k,v in ipairs(getPlayersInTeam( getTeamFromName("Law Enforcement") )) do
			if (exports.GTIemployment:getPlayerJob(v,true) == "Police Officer") then
				...
				if (getDistanceBetweenPoints3D(x,y,z,cX,cY,cZ) <= MIN_SUICIDE_DISTANCE) then
					table.insert(cops,v)
				end
			end
		end
	...
	if not isTimer(timers[player]) then
		if (#cops >= 1) then
			return message:dm("You can't commit suicide while a Police Officer is nearby.",player,255,0,0)
		end
